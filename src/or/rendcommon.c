...
void print_hex(const char * str, int len, char * prev)
{
	char hex_str[2*len+1];
	base16_encode(hex_str, 2*len+1, str, len);
	if(prev)
		log_warn(LD_GENERAL,"%s%s",prev,hex_str);
	else
		log_warn(LD_GENERAL,"%s",hex_str);
                        ...
                        uint8_t replica)
{
  ...
  /* Calculawwte current time-period. */
                 ...
                 const char *service_id)
{
  ...
  log_warn(LD_GENERAL,"Here: %s", desc);
  rend_cache_entry_t *e;
