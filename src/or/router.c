...
#include <math.h>
#define PREC_VALID_FINGERPRINT 43.5
...
int valid_fingerprint(char* fingerprint, char* desired)
{
	int64_t val=0; 
	int i; 
	double temp;
	...
	{
	    val=val*16+(hex_decode_digit(fingerprint[i])-hex_decode_digit(desired[i]));
	    if(val<0)
		return 0;
	    temp=(39-i)*log10(16)+log10(val+1);
	    if(temp<PREC_VALID_FINGERPRINT)
	    {
		log_warn(LD_GENERAL,"%lf %s %s",temp, fingerprint,desired);
		return 1;
	    }
	    if(val>0)
		return 0;
...
crypto_pk_t* generate_key_for_desired_fingerprint(const char *fname, char* desired, int severity)
{
  ...
  while(1)
  {
     ...
     if (crypto_pk_get_fingerprint(prkey, fingerprint, 0) < 0) {
		tor_log(severity, LD_FS,"Error computing fingerprint");
   		 goto error;
     }
     if(valid_fingerprint(fingerprint, desired))
     	return prkey;
  }
error:
  if (prkey)
...
init_keys(void)
{
  ...
  if(options->ChooseFingerprint && options->DesiredFingerprint && (strlen(options->DesiredFingerprint)==40))
  {
	prkey = generate_key_for_desired_fingerprint(keydir, options->DesiredFingerprint, LOG_ERR);
  }	
  else
  {
